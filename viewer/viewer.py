...
class Viewer:
    ...
    """
    def __init__(self,box_type = "OpenPCDet",bg=(0, 0, 0)):
    ...
    def save_camera(self):
        self.camera_params = {
            "clipping_range": self.vi.camera.GetClippingRange(),
            "focal_point": self.vi.camera.GetFocalPoint(),
            "position": self.vi.camera.GetPosition(),
            "view_angle": self.vi.camera.GetViewAngle(),
            "view_up": self.vi.camera.GetViewUp(),
            "direction":self.vi.camera.GetDirectionOfProjection(),
            "distance":self.vi.camera.GetDistance(),
            "thickness":self.vi.camera.GetThickness(),
            "plane":self.vi.camera.GetViewPlaneNormal(),
            
        }

    def show_3D(self, keep_view = True):
        """
        ...
        :return:
        """
        if keep_view and self.camera_params is not None:
            self.vi.show(self.actors+self.actors_without_del,resetcam=False, camera={'pos':self.camera_params["position"],'focalPoint':self.camera_params["focal_point"],"clip":self.camera_params["clipping_range"],"viewangle":self.camera_params["view_angle"]})
        else:
            self.vi.show(self.actors+self.actors_without_del,resetcam=False, camera={'pos': (-100, 0, 280), 'focalPoint': (5, 0, 2), 'viewup': (0, 0, 100)})
        self.save_camera()
        self.vi.clear()
        self.actors.clear()
